...
function build(creep) {
  ...
  if (building) {
    if (creep.room.name === Memory.homeRoomName) {
      target = Game.getObjectById("61c24d75bb403a054582507a");
      ...
      target = Game.getObjectById("61c83e8a3762ce546612aee7");
      if (target && target.progress < target.progressTotal) {
        // good, keep target
        creep.memory.lastBuildID = target.id;
      } else {
      ...
      // tower construction site
      target = Game.getObjectById("61cbf907be69f6fc960e64f5");
      if (target && target.progress < target.progressTotal) {
        // good, keep target
        creep.memory.lastBuildID = target.id;
      } else {
