...
function spawnCreepTypes(enAvail, spawns) {
  ...
  //  #     #   #   #    #   #
  //   #####     ###      ###
  if (enAvail >= 300 && !invaderId && creepsHome.length < 5) {
    let name = "h" + t;
    let chosenRole = "h";
    let direction = "home";
    let sourceId = Memory.source2;
  ...
  if (enAvail >= 550 && !invaderId) {
    ...
    if (roadRepairersSouth.length < 4 && !sAttackerId) {
      logConditionPassedForSpawnCreep(
        "roadRepairersSouth",
        roadRepairersSouth,
        4
    ...
    } else if (harvestersSouth.length < 4 && !sAttackerId) {
      logConditionPassedForSpawnCreep("harvestersSouth", harvesters, 4);
      name = "hS" + t;
      chosenRole = "h";
      direction = "south";
      parts = slowMoverParts550;
    ...
    } else if (harvestersSouthwest.length < 6 && !sAttackerId) {
      logConditionPassedForSpawnCreep(
        "harvestersSouthwest",
        harvestersSouthwest,
        6
